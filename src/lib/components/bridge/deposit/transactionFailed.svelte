...
<div class="flex flex-col p-2">
  <div class="flex items-center justify-center my-4">
    <!-- Error -->
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="fill-red-500 w-24 h-24">
  ...
  <div class="flex items-center justify-center p-2 pb-0">
    <span class="font-semibold text-base">{$_('bridge.an_error_occurred')}</span>
  </div>
  <div class="flex items-center justify-center p-2 select-text">
    <span class="font-medium break-all text-sm"> { $errorMessage } </span>
  </div>
  <div class="back flex justify-center mt-4 gap-x-4">
    <button
    ...
    >
      <span class="text-base-content"> {$_("bridge.close")} </span>
    </button>
    <button
      on:click={() => mode.set(get(errorLastMode))}
      class="btn bg-blue-700 border-base-200 hover:bg-blue-800 hover:border-base-300 rounded-2xl"
    >
      <span class="text-base-100"> {$_("bridge.retry")} </span>
    </button>
  </div>
</div>