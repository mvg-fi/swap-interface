...
<div class="flex flex-row items-center justify-between space-x-4">
  <button 
    on:click={()=> $process-=1}
    class={clsx("flex items-center space-x-2","btn rounded-2xl bg-base-100 border-none hover:bg-slate-300 [[data-theme=dark]_&]:hover:bg-slate-700 no-animation", $process<=0&&"disabled")}
  >
    <svg xmlns="http://www.w3.org/2000/svg" class="fill-base-content h-5" viewBox="0 0 24 24" height="24" width="24">
  ...
  {#if $process != 4}
    ...
    >
      <span class="text-base-100 [[data-theme=dark]_&]:text-base-content mr-2">{$_('pool.next')}</span>
  ...
  {:else}
    <button 
      ...
      class={clsx("flex items-center justify-center","btn rounded-2xl bg-primary border-none hover:bg-primary no-animation",
       !valid &&"btn-disabled bg-slate-200 [[data-theme=dark]_&]:bg-slate-600")}
    >
