...
{#each coins as coin, i}
  <div class="p-1 m-1 border-solid border-2 rounded-2xl">
    ...
    <div class="flex flex-row mx-2 my-1 opacity-75 text-xs">
      ...
      <div on:click={ ()=>{setMax(balance[i],i)} } class="tooltip tooltip-left" data-tip={$_('add_liquidity.max')}>
        <span class="cursor-pointer">{$_("add_liquidity.balance")}: {balance[i]}</span>
      </div>
    </div>
  </div>
{/each}
